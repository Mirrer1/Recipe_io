...
router.post('/', isLoggedIn, async (req, res, next) => {
  ...
  try {
    ...
    if (req.body.content.tags && req.body.content.tags.length !== 0) {
      const hashtags = req.body.content.tags
        .split(/(#[^\s#]+)/g)
        .map((v, i) => {
          if (v.match(/(#[^\s#]+)/)) {
            return v;
    ...
    if (req.body.imagePaths) {
      if (Array.isArray(req.body.imagePaths)) {
        const images = await Promise.all(
          req.body.imagePaths.map((image) => Image.create({ src: image }))
        );
...
router.post(
  ...
  upload.array('image'),
  async (req, res, next) => {
    // uploadImagesAPI / POST /post/images
    try {
      res.json(
...
router.patch('/:postId/edit', isLoggedIn, async (req, res, next) => {
  // editPostAPI / PATCH /post/1(게시글 ID)/edit
  try {
    await Post.update(
      {
        title: req.body.content.title,
    ...
    if (req.body.fullEditImagePaths) {
      if (Array.isArray(req.body.fullEditImagePaths)) {
        const result = await Promise.all(
          req.body.fullEditImagePaths.map((image) =>
            Image.findOrCreate({
              where: { src: image },
            })
          )
...
router.post('/:postId/comment', isLoggedIn, async (req, res, next) => {
  // addCommentAPI / POST /post/1(게시글 ID)/comment
  try {
    const post = await Post.findOne({
      where: { id: req.params.postId },
      include: [
...
router.patch('/:postId/comment', isLoggedIn, async (req, res, next) => {
  // editCommentAPI / PATCH /post/1(게시글 ID)/comment
  try {
...
router.delete('/comment', isLoggedIn, async (req, res, next) => {
  // removeCommentAPI / DELETE /post/comment
  try {
    await Alert.destroy({
      where: {
        type: 'comment',
...
router.patch('/:postId/like', isLoggedIn, async (req, res, next) => {
  // likePostAPI / PATCH /post/1(게시글 ID)/like
  try {
    const post = await Post.findOne({
      where: { id: req.params.postId },
      include: [
...
router.delete('/:postId/like', isLoggedIn, async (req, res, next) => {
  // unLikePostAPI / DELETE /post/1(게시글 ID)/like
  try {
    const post = await Post.findOne({
      where: { id: req.params.postId },
    });
...
router.delete('/:postId', isLoggedIn, async (req, res, next) => {
  // removePostAPI / DELETE /post/1(게시글 ID)
  try {
    await Alert.destroy({
      where: {
        PostId: req.params.postId,
