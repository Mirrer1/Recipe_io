...
import { MypageWrapper, MypageText, PageMainText, PageSubText, LogoutResult } from '../styles/pageStyles';

const profile = () => {     
  const { me, nicknameEditDone, nicknameEditError } = useSelector((state) => state.user);        
  ...
  useEffect(() => {
    nicknameEditDone && message.success('닉네임이 정상적으로 변경되었습니다.', 1.5);
    ...
    <>
      ...
      <AppLayout>
        <MypageWrapper>
          ...
          {me ? <MyInfo /> : <LogoutResult title="유저의 정보가 존재하지 않습니다." extra={<div>로그인 후 이용해주세요.</div>}/>}
        </MypageWrapper>
      </AppLayout>
    </>
  )
