...
import {
  RESET_REMOVE_POST_STATE,
  RESET_LIKE_STATE,
  RESET_UNLIKE_STATE,
  RESET_ADD_COMMENT_STATE,
  RESET_REMOVE_COMMENT_STATE,
} from '../../reducers/post';

const AppLayout = ({ children }) => {
  const { me, logInDone, logInError, logOutDone, contactFormVisible } =
    useSelector((state) => state.user);
  const {
    singlePost,
    postDeleteModalVisible,
  ...
  useEffect(() => {
    addCommentDone &&
      message.success('해당 게시글에 댓글이 작성되었습니다.', 1.5);
  ...
  useEffect(() => {
    removeCommentDone &&
      message.error('정상적으로 댓글이 삭제되었습니다.', 1.5);
  ...
  return (
    <>
      <div ref={topPageRef}>{me ? <LoginMenu /> : <LogoutMenu />}</div>
      <MenuDivider />
      {children}
      {showBtn && (
        <UpBtn onClick={moveToMenuBtn}>
          <UpBtnIcon />
        </UpBtn>
      )}
