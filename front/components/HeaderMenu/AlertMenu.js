      ...
      <Row justify='space-between'>
        <Col>
          <AlertText onClick={onClickAlert(userAlert)}>
            ...
            <AlertUserNickname className='bolder'>
              {userAlert.Alerter?.nickname}
            </AlertUserNickname>
            님이&nbsp;
            <span className='bolder'>
              {userAlert.Post?.title.length > 10
                ? userAlert.Post?.title.slice(0, 10) + '...'
                : userAlert.Post?.title}
            </span>
            &nbsp; 게시글에{' '}
            {userAlert?.type === 'like'
              ? '좋아요를 눌렀습니다.'
        ...
        <Col>
          <AlertDeleteBtn
            onClick={onDeleteAlert(userAlert)}
